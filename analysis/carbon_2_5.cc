...
void carbon_2_5()
{
    ...
    double energies[n_values] = {398.8,375.8,349.9,324.1,275.6,257.5,250.5,236.2,224,200.6,175.4};
    ...
    for(int run_number=0; run_number<n_values; run_number++)
    {
        ...
        TFile *file = new TFile("../output_files/carbon_2_5_run_"+n_run+".root");
        ...
        scoring->Draw("pre_Ek/12>>his1", "pre_Ek>0");
        ...
        scoring->Draw("fEdep/12>>his3", "fEdep>0");
        ...
        his3->Draw();
        c5->SaveAs(Form("fit_plot/carbon_2_5/fit%d.png",run_number));
    ...
    g_edepMedian->SetFillColor(kGreen);
    g_edepMedian->SetFillStyle(3001);
    g_edepMedian->Draw("AP3");
    ...
    g_track_len->SetTitle("Mean distance inside the LYSO;E_{K} before LYSO per nucleon [MeV]/N; distance in LYSO [mm]");
    g_track_len->SetMarkerColor(4);
    g_track_len->SetMarkerStyle(21);
    g_track_len->Draw("AP");*/

    TFile *output = new TFile("./carbon_2_5.root","RECREATE");
    g_edepMean->Write();
    g_edepMode->Write();
    g_edepMedian->Write();
    //g_track_len->Write();
