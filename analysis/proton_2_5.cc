...
void proton_2_5()
{
    ...
    double energies[n_values] = {130.6,129.4,128.2,127,125.7,124.5,123.3,122,120.7,119.5};
    ...
    for(int run_number=0; run_number<n_values; run_number++)
    {
        ...
        TFile *file = new TFile("../output_files/proton_2_5_run_"+n_run+".root");
        ...
        his3->Draw();
        c5->SaveAs(Form("fit_plot/proton_2_5/fit%d.png",run_number));
    ...
    /*TCanvas *c1 = new TCanvas("c1","Mean kinetic energy (p) before LYSO",600,600);
    TGraphErrors *g_pre_Ek = new TGraphErrors(n_values,energies,pre_Ek_means,energies_err,pre_Ek_std_dev);
    g_pre_Ek->SetTitle("Mean kinetic energy before LYSO;starting E_{K} [MeV];E_{K} before LYSO [MeV]");
    g_pre_Ek->SetMarkerColor(4);
    g_pre_Ek->SetMarkerStyle(21);
    g_pre_Ek->Draw("AP");

    TCanvas *c2 = new TCanvas("c2","Mean kinetic energy (p) after LYSO",600,600);
    TGraphErrors *g_post_Ek = new TGraphErrors(n_values,energies,post_Ek_means,energies_err,post_Ek_std_dev);
    g_post_Ek->SetTitle("Mean kinetic energy after LYSO;starting E_{K} [MeV];E_{K} after LYSO [MeV]");
    g_post_Ek->SetMarkerColor(4);
    g_post_Ek->SetMarkerStyle(21);
    g_post_Ek->Draw("AP");

    TCanvas *c3 = new TCanvas("c3","Mean energy deposition",600,600);
    TGraphErrors *g_edep = new TGraphErrors(n_values,energies,edep_means,energies_err,edep_std_dev);
    g_edep->SetTitle("Mean energy deposition;starting E_{K} [MeV];E deposited in LYSO[MeV]");
    g_edep->SetMarkerColor(4);
    g_edep->SetMarkerStyle(21);
    g_edep->Draw("AP");*/
    ...
    TFile *output = new TFile("./proton_2_5.root","RECREATE");
    g_edepMean->Write();
    g_edepMode->Write();
    g_edepMedian->Write();
    //g_track_len->Write();
