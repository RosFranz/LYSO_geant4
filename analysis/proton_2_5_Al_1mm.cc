...
void proton_2_5_Al_1mm()
{
    gStyle->SetLineWidth(2);
    const int n_values = 13;
    const int n_bins = 240;
    const int n_quantiles = 3;

    double energies[n_values] = {228,220,202,179,154,143,131,125,119,112,106,91,70};
    ...
    for(int run_number=0; run_number<n_values; run_number++)
    {
        ...
        TFile *file = new TFile("../output_files/proton_2_5_Al_1mm_run_"+n_run+".root");
        ...
        his3->Draw();
        c5->SaveAs(Form("fit_plot/proton_2_5_Al_1mm/fit%d.png",run_number));

        edep_means[run_number] = fit->GetMaximumX(x_left, x_right, 0.001);
        edep_std_dev[run_number] = (abs(fit->GetMaximumX(x_left, x_right, 0.001)-his3->GetBinCenter(his3->GetMaximumBin())) + fit->GetParError(2))/2; //linear sum of the errors
        */
    ...
    TFile *output = new TFile("./proton_2_5_Al_1mm.root","RECREATE");
    g_edepMean->Write();
    g_edepMode->Write();
    g_edepMedian->Write();
    //g_track_len->Write();
